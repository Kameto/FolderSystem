...
{	
	...
	DrawBox(0, (((PicData::GetDataNum() / 4) + 1) * 168) - scrool, 672, (((PicData::GetDataNum() / 4) + 1) * 168) + 16 - scrool, 0x800000, true);
	DrawBox(0, (((PicData::GetDataNum() / 4) + 1) * 168) - scrool, 672, (((PicData::GetDataNum() / 4) + 1) * 168) + 16 - scrool, 0x000000, false);
	...
	for (int i = 0, n = PicData::GetDataNum(), j = 0; i < n; i++)
	{
		...
		if (picFlag[i] == true)
		{
			...
			DrawExtendFormatString(680, 440, 0.70, 0.70, 0x000000, "アイテム番号:%s Lock:%s", str.number.c_str(), str.classification.c_str());
